...
Exemples :
    ...
    # Dans le conteneur spark, seul runtime Spark du projet :
...
def creer_session(config: argparse.Namespace) -> SparkSession:
    ...
    * la mémoire relève de `spark-submit` (`--driver-memory`), fixée par le
      `docker-compose.yml` : en `local[*]` tout le travail se fait dans
...
def _est_conforme() -> Column:
    ...
    Le piège de `from_json` : en mode permissif (le défaut), un JSON illisible ne donne
...
def superviser(spark: SparkSession, requetes: list[Any], arret: dict[str, bool]) -> None:
    ...
    Ce chemin ne sert que si le script est lancé directement (`python
