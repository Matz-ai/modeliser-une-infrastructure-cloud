...
> Conception d'une **infrastructure de données hybride** (on-premise ↔ cloud) et POC d'un
...
L'entreprise fictive **InduTech** veut moderniser sa gestion de données (IoT, +50 Go/mois en temps
réel) en s'appuyant sur le cloud **sans casser** son SI on-premise existant (SQL Server, SAN,
Active Directory, ERP/CRM). Le projet se découpe en deux exercices indépendants :

| | Exercice | Nature | Livrables |
|---|---|---|---|
| **1** | Modélisation d'une infrastructure hybride | **Sur le papier** — rien n'est déployé | [Schéma d'architecture](docs/exercice1-modelisation/) + [évaluation de compatibilité](docs/exercice1-modelisation/evaluation-compatibilite.md) |
...
> L'Exercice 2 **simule** l'usage cloud en local : c'est ce que demande explicitement la consigne
...
flowchart LR
    ...
    subgraph LD["③ LOAD — exports"]
        PARQ["🗂️ <b>Parquet</b><br/>exports/tickets_enrichis/<br/>partitionné par request_type"]
        JSONAGG["📊 <b>JSON</b><br/>exports/agregats/<br/>4 vues métier"]
        CONS["🖥️ <b>Insights en console</b><br/>3 tableaux toutes les 10 s"]
...
> **Ni Python, ni Java, ni Spark ne sont requis sur votre machine.** Tout s'exécute dans les
> conteneurs : l'image Spark officielle embarque déjà le JRE, PySpark et — depuis notre Dockerfile —
> les JARs du connecteur Kafka. C'est un choix délibéré : le POC démarre sur un poste vierge.
...
Au bout de ~30 secondes, les logs affichent des tableaux d'insights qui se rafraîchissent **toutes
les 10 secondes** :
...
Le premier arrête tout en **conservant** le topic et les exports. Le second ajoute la suppression
des volumes : on repart d'un état totalement vierge.

> ℹ️ Le service `spark` sort en **code 143** à l'arrêt : c'est normal. Sous `spark-submit`, le
> processus n°1 du conteneur est la JVM, qui intercepte le signal d'arrêt. Aucune donnée n'est
> perdue — les checkpoints sont validés à chaque micro-batch.
...
**Une agrégation** (`request_type` × `equipe_support` × `priority`), dont **quatre vues** sont
exportées :
...
> **Pourquoi deux formats.** Le **Parquet** est colonnaire, compressé et typé : il se lit
> directement dans pandas, DuckDB, Power BI ou Athena — c'est la « visualisation ultérieure »
> demandée par la consigne. Le **JSON** des agrégats est petit et lu par un humain : il s'ouvre
...
Tout ce qui circule entre les composants est figé dans deux documents — c'est ce qui a permis de
développer le producteur, le traitement et la conteneurisation **en parallèle** :
...
| `File …/15.delta does not exist` puis arrêt de Spark | Un arrêt brutal est tombé pile pendant l'écriture de l'état des agrégations : le point de reprise est incomplet. | `rm -rf data/checkpoints/agregats` puis relancer. Le traitement repart du début du topic — aucune donnée perdue, elle est toujours dans Redpanda. |
...
> 🔴 **À tourner par Mathieu** — script et déroulé prêts dans
> [`docs/script-video.md`](docs/script-video.md).
> Le lien Loom sera intégré ici.

**▶️ [Démonstration du pipeline ETL (Loom)](https://www.loom.com/share/LIEN-A-REMPLACER)**
...
├── docs/
│   ├── plan-conversations.md       # Plan de travail collaboratif
│   ├── contrat-ticket.md           # Contrat partagé : ticket, topic, broker
│   ├── contrat-export.md           # Contrat partagé : exports, checkpoints, runtime Spark
│   ├── script-video.md             # Script et déroulé de la vidéo de démonstration
│   ├── journal/                    # Journal d'avancement, un fichier par conversation
